...
ENI_Bal <- subset(ENI, ENI$fips == "24510")

## Create a dataframe having yearwise values for Emissions for Baltimore from all the sources ("SCC")    
all_emissn <- aggregate(Emissions~year, ENI_Bal, sum)
...
Period <- as.numeric(levels(as.factor(ENI_Bal$year)))

## Plot Tot_emiss and Period withouth the axes which we will add later      
plot(Period,Tot_emiss,col = "red", type = "l", xlab = "Years", ylab = "Total Emissions from PM2.5(in thousand Tons)", xaxt='n', ylim=c(0,5))
...
## Add summary of the data "Total Emissions from PM2.5 have decreased in Baltimore City Maryland from 1999 to 2008"
mtext("PM2.5 Emissions decreased in Baltimore City from 1999 to 2008", outer = TRUE)
