...
def dfs_loop(array0,record,order):
    global t,s,finish_times_seen,finish_times,leaders
    t=0
    s=None
               ...
               # print 'search through all its outgoing vertex'
                for j in range(start,end+1):
                  ...
                  #  print 'neighbor is ',nn
                    if record[nn-1,1]==0 and nn not in S_set:#if nn hasn't been explored, go to it next
                      ...
                      #  print 'neighbor',nn,'has been visited'
                        ...
                        while len(pred)>0 and brk!=1:
                           ...
                           # print 'retrieve to last step ',last,' and check its neighbours'
                            last_start=record[last-1,2]
                            last_end=record[last-1,3]
...
#                lst.append(line)
    ...
    record = np.column_stack((np.array(list(vertex_set)),np.zeros([n,3],dtype=int)))
