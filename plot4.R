...
## For emission from Coal Combustion-related sources, subset the SCC dataframe matching "Coal" string in the column EI.Sector      
SCC_Coal <- subset(SCC, grepl("Coal", EI.Sector))

## Now subset the ENI dataframe so that it has only those SCC-codes which are within the SCC_Coal dataframe  
ENI_Coal <- subset(ENI, SCC %in% SCC_Coal$SCC)

## R-Code For "plot4"

## Create a dataframe that having yearwise values for Emissions for all the counties ("fips") and all the sources ("SCC")    
all_emissn <- aggregate(Emissions~year, ENI_Coal, sum)

## print all_emissn and check the magnitude of the values under the Emission    
all_emissn 

## Create a vector "Tot_emiss" and assign to it the values under the all_emissn$Emission/10^3 
Tot_emiss <- all_emissn$Emission/10^3

## Create a vector "Period" and assign to it the unique values under the column "year"   
Period <- levels(as.factor(ENI_Coal$year))

## Plot Tot_emiss and Period withouth the axes which we will add later      
plot(Period,Tot_emiss,col = "red", type = "l", xlab = "Years", ylab = "Coal Combustion PM2.5 Emissions (in thousand Tons)", xaxt='n', ylim=c(0,750))
...
## Add summary of the data "PM2.5 Emissions from Coal Combustion decreased in US from 1999 to 2008"
mtext("PM2.5 Emissions from Coal Combustion decreased in US from 1999 to 2008", outer = TRUE)
