## cachematrix.R ---> by Sanjana Sri
## My aim in this experiment is to write a pair of functions, namely,
#"makeCacheMatrix" and "cacheSolve" that cache the inverse of a matrix

## This function Function makeCacheMatrix gets a
#matrix as an input, set the value of the matrix,
...
## The function cacheSolve takes the output of the previous matrix makeCacheMatrix(matrix) as an input and checks inverse matrix  # nolint: line_length_linter.
# from makeCacheMatrix(matrix) has any value in it or not.
# In case inverse matrix from makeCacheMatrix((matrix) is empty, it gets the
# original matrix data from and set the invertible matrix by using the solve function. # nolint: line_length_linter.
# In case inverse matrix from makeCacheMatrix((matrix) has some value in it
# (always works after running the code 1st time), it returns a message
