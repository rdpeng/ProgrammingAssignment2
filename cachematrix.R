...
##  cacheSolve can do it either (1) returning the inverted matrix stored in getinv or 
##  (2) calculating it with a solve function and passing the arguments to setinv.

cacheSolve <- function(x, ...) {
         ##  Firstly, if there is an inverse matrix already stored in the cache cacheSolve recovers it
             m <- x$getinv()
                if(!is.null(m)) {
                        message("Getting data from cache")
                        return(m)
                }
        ## The next is a sort of else{} and return a matrix that is the inverse of 'x'
