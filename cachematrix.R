# Function create, store and recall a matrix and its inverse in/from cache  

makeCacheMatrix <- function(x = matrix()){
  #creates custom matrix type capable of running four functions
  
  m <- NULL
  set <- function(y){ #stores the matrix in cache, get recalls the matrix
    x <<- y  
...
# cacheSolve take a custom matrix type created by the makeCacheMatrix function and calculates the inverse matrix of it but first it checks to see if the calculation has been done before if it has been done before it recalls the data from the cache. If it has not been done before it calculates the inverse matrix then store it in the cache
